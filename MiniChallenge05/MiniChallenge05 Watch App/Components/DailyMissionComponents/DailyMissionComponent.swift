    ...
    var body: some View {
        VStack {
            ...
            HStack {
                ...
                DailyMissionItemComponent(image: "Binóculo", missionItem: dailyMissionList.first?.dailyMission.missionItemAlpha ?? 0, currentItem: dailyMissionList.first?.itemAlpha ?? 0, missionCompletion: ((dailyMissionList.first?.missionCompletion) ?? false))
                
                Spacer()
                
                DailyMissionItemComponent(image: "Camera", missionItem: dailyMissionList.first?.dailyMission.missionItemBravo ?? 0, currentItem: dailyMissionList.first?.itemBravo ?? 0, missionCompletion: ((dailyMissionList.first?.missionCompletion) ?? false))
                
                Spacer()
                
                DailyMissionItemComponent(image: "Folder", missionItem: dailyMissionList.first?.dailyMission.missionItemCharlie ?? 0, currentItem: dailyMissionList.first?.itemCharlie ?? 0, missionCompletion: ((dailyMissionList.first?.missionCompletion) ?? false))
                
                Spacer()
                
                DailyMissionItemComponent(image: "Hat", missionItem: dailyMissionList.first?.dailyMission.missionItemDelta ?? 0, currentItem: dailyMissionList.first?.itemDelta ?? 0, missionCompletion: ((dailyMissionList.first?.missionCompletion) ?? false))
